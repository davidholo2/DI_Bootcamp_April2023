...
class MenuItem:
    ...
    def delete(self):
        cursor.execute(
            f"SELECT * FROM menu_items WHERE item_name = '{self.name}' AND item_price = {self.price};")
    ...
    def update(self, new_name, new_price, old_name, old_price):
        cursor.execute(
            f"SELECT * FROM menu_items WHERE item_name = '{old_name}' AND item_price = {old_price};")
        if len(cursor.fetchall()) == 0:
            print("No items found")
        else:
...
def connect_sql():
    ...
    PASSWORD = '1234'
    DATABASE = 'menu'
    connection = ps.connect(host=HOSTNAME, user=USERNAME,
